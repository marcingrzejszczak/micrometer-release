...
class ReleaseDateCalculatorTests {
    ...
    @ParameterizedTest(name = "{index} For current date <{0}> and version <{1}> expected deadline date should be <{2}>")
    @CsvSource(textBlock = """
            2025-01-01, 1.0.0, 2025-01-13
            2025-01-14, 1.0.0, 2025-02-10
            2025-01-01, 1.0.0-M1, 2025-01-13
            2025-01-14, 1.0.0-M1, 2025-02-10
            """)
    void should_calculate_next_milestone_date(LocalDate now, String version, LocalDate expectedDeadline) {
        log.info("Current date [{}] is [{}]", now, now.getDayOfWeek());
        log.info("Expected date [{}] is [{}]", expectedDeadline, expectedDeadline.getDayOfWeek());
        LocalDate milestoneDate = ReleaseDateCalculator.calculateDueDate(now, version);
