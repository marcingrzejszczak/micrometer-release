...
class ProjectTrainPostReleaseTaskScheduler {

    private static final Logger log = LoggerFactory.getLogger(ProjectTrainPostReleaseTaskScheduler.class);

    private final PostReleaseWorkflow postReleaseWorkflow;

    private final String githubOrgRepo;

    ProjectTrainPostReleaseTaskScheduler(PostReleaseWorkflow postReleaseWorkflow, String githubOrgRepo) {
    ...
    void runPostReleaseTasks(List<String> versions) {
        versions.sort(Comparator.comparing(ProjectTrainPostReleaseTaskScheduler::extractMajorMinorVersion));
