...
class GithubActionsAcceptanceTests {
    ...
    static void should_go_through_whole_flow() throws InterruptedException {
        Assumptions.assumeTrue(System.getenv("GH_TOKEN") != null, "Github Token is mandatory");
        resetMilestones();
    ...
    void should_verify_milestone_0_0_2() throws JsonProcessingException {
        ...
        assertThat(issues)
            ...
            .containsOnly("Closed issue in generic", "Closed bug in concrete", "Closed bug in generic", "Closed enhancement in generic");
    }
    ...
    void should_verify_milestone_0_0_3() throws JsonProcessingException {
        ...
        assertThat(issues)
            .extracting(Issue::title)
            .containsOnly("Open issue in concrete");
    }
    ...
    private static void resetMilestones() throws InterruptedException {
        log.info("Running reset milestones action");
        processRunner.run("gh", "workflow", "run",
            "reset-milestones.yml");
    ...
    private static void runPostReleaseWorkflow() throws InterruptedException {
        ...
        processRunner.run("gh", "workflow", "run",
            ...
            "--ref", "v0.0.2",
            "-f", "previous_ref_name=v0.0.1");
        waitForWorkflowCompletion("post-release-workflow.yml");
    }

    private static void waitForWorkflowCompletion(String workflowFile)
        throws InterruptedException {
        log.info("Waiting for workflow [{}] completion...", workflowFile);
        Thread.sleep(10_000); // Wait for the action to schedule
        boolean completed = false;
        int attempts = 0;
        while (!completed && attempts < 30) { // 5 minute timeout
            List<String> status = processRunner.run("gh", "run", "list", "--workflow", workflowFile,
                "--limit", "1");
            log.info("Workflow [{}] not completed yet...", workflowFile);
            completed = status.stream()
                .anyMatch(line -> line.contains("completed"));
            if (!completed) {
                Thread.sleep(10_000);
                attempts++;
            }
    ...
    static class GithubClient {
        ...
        GithubClient(String token, String repo) {
            this.repo = repo;
            this.processRunner = new ProcessRunner(repo)
                .withEnvVars(Map.of(
                    "JAVA_HOME", JavaHomeFinder.findJavaHomePath(),
                    "GH_TOKEN", token != null ? token : ""
                ));
        ...
        List<Issue> getClosedIssuesForMilestone(int milestoneNumber)
